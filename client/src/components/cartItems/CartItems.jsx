    ...
    <div className="cart-item">
      ...
      <div className="cart-item-right">
        ...
        <div className="item-cost">
          <LiaPoundSignSolid className="icon" />
          {cartItem.price}
        </div>
        <motion.div
          whileTap={{ scale: 0.75 }}
...
export default function CartItems() {
  ...
  const handlePlaceOrder = () => {
    if (cart.cartItems.length) {
      // Extract the restaurantId from the first item in the cart (assuming all items have the same restaurantId)
      const restaurantId = cart.cartItems[0].restaurantId;

      dispatch(createOrder(token, cart.cartItems, restaurantId)); // pass restaurantId with other order details
    } else {
      alert("Empty cart");
    }
  };
    ...
    <div className="cart-items">
      ...
      <div className="cart-bottom">
        {!!cart.cartItems.length && (
          <>
            <p>
              <span>Total:</span> <LiaPoundSignSolid className="icon" />
              {cart.totalCost}
            </p>
