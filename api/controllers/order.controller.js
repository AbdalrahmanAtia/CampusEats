...
export const getAllUserOrders = asyncHandler(async (req, res) => {
  ...
  export const getAllOrders = asyncHandler(async (req, res) => {
    ...
    const orders = await OrderModel.getAllOrdersByRestaurant(restaurantId);
  
    if (!orders) {
      throw new ApiError(500, "Error while getting orders ");
    }
  
    const filteredOrders = await transformUserOrderData(orders);
